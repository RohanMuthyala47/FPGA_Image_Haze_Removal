        ...
        for j in range(raw_trans.shape[1]):
            ...
            if np.sum(edge_patch) > 0:
                ...
                filtered_patch = cv2.bilateralFilter(
                    patch.astype(np.float32), d=5, sigmaColor=0.05, sigmaSpace=5
                )
...
def dehaze_image(hazy_img, patch_size=5, omega=0.85, t0=0.1, r=5, eps=1e-5):
    ...
    return dehazed_img, refined_transmission, raw_transmission


# Load the hazy image
# r"C:\Users\Rohan\Documents\Images\canyon_512.bmp"
# r"C:\Users\Rohan\Downloads\archive\RESIDE-6K\training\hazy\43.jpg"
hazy_img = cv2.imread(r"C:\Users\Rohan\Downloads\0001_0.85_0.04.jpg")
hazy_img = cv2.cvtColor(hazy_img, cv2.COLOR_BGR2RGB)
hazy_img = hazy_img.astype(np.float32) / 255.0

# Dehaze
dehazed_img, transmission_map, raw_transmission = dehaze_image(hazy_img)
